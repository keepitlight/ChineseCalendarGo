...
// Hour returns the notation(sexagesimal cycle) of the hour by the major of day.
//
// 根据日干返回时辰的干支，例如凌晨九点，早上九点，下午九点等
func Hour(hour int, day Major) Notation {
	j := MajorOfHour(hour, day)
	i := MinorOfHour(hour)
	if n, p := Pair(j, i); !p {
		return Invalid
	} else {
